...
class App(QWidget):
    ...
    def parse_everything(self):
        ...
        try:
            ...
            self.log(f'Minimize: {type(self.target)} {self.target}')
            self.target *= -1
            for inp in self.constraints_input:
    ...
    def on_simulation_clicked(self):
        if not self.started:
            ...
            self.prev = None
            self.k = 0
            print('\n!!!!!!!!!!!!\nself.x_array', self.x_array, type(self.x_array), type(self.x_array[0]), '\nself.x0_input', self.x0_input, type(self.x0_input), '\n!!!!!!!!!!!!\n')
        while True:
